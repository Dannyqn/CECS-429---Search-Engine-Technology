...
public class BooleanQueryParser {
	...
	private Literal findNextLiteral(String subquery, int startIndex) {
		...
		if(subquery.charAt(startIndex) == '\"') { // phrase literal
			...
			List<Query> qList = new ArrayList<Query>(); //create a query list
			...
			for(String s: TermList) { //for each term add the word to a queryList and make a separate term for each of them
				qList.add(new TermLiteral(s, new AdvancedTokenProcessor()));
			}
			
			return new Literal(new StringBounds(startIndex, nextQuote),new PhraseLiteral(qList)); //return a new literal taking in a PhraseLiteral
		}
		else if(subquery.charAt(startIndex) == '-') { // NOT operator
			
			Literal lit = findNextLiteral(subquery.substring(0),++startIndex); // find the next literal skipping the "-"
			
			List<Query> qList = new ArrayList<Query>();
			qList.add(lit.literalComponent); //add the literal to a queryList
			
			return new Literal(lit.bounds, new NotQuery(qList)); // return a new literal taking in a NotQuery
		}
		else {
		...
		return new Literal(
		 ...
		 new TermLiteral(subquery.substring(startIndex, startIndex + lengthOut),new AdvancedTokenProcessor()));
